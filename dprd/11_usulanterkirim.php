...
$p_id_admin       = isset($_POST['admin']) ? $_POST['admin'] : "";
$p_tgl_verifikasi = isset($_POST['tgl_verifikasi']) ? $_POST['tgl_verifikasi'] : "";
$p_stat_verifikasi         = isset($_POST['stat_ver'])? $_POST['stat_ver'] :"";
$p_tombol         = isset($_POST['kirim_daftar']) ? $_POST['kirim_daftar'] : "";
$p_submit         = "KIRIM";
$mod              = isset($_GET['mod']) ? $_GET['mod'] : NULL;
$id_kirim         = isset($_GET['id']) ? $_GET['id'] : NULL;


//window.location=('11_usulanterkirim.php');
?>
...
<body>
  <section id="container">
    <header class="header black-bg">
     ...
     <section id="main-content">
      <section class="wrapper">
        ...
        <h3><i class="fa fa-angle-right">Usulan Terkirim</i></h3>
        <div class="row mb">
    ...
    <th class='hidden-phone'>Nama Pengusul</th> 
    ...
    <?php
    $q_usulan   = mysql_query("SELECT * FROM t_usulterkirim INNER JOIN t_rekapusulan ON t_rekapusulan.id_usulan = t_usulterkirim.id_usulan WHERE stat_verifikasi ='Belum Verifikasi' ORDER BY id_kirim ASC") or die (mysql_error());
    ...
    if ($j_data == 0) {
      ...
      while ($a_usulan = mysql_fetch_array($q_usulan)) {
        echo "<tr>                          
        <td >$a_usulan[stat_verifikasi]</td>
        <td >$a_usulan[usulan]</td>
        <td >"; getkecamatan($a_usulan['id_kec']); echo "</td> 
        <td >"; echo getdesa($a_usulan['id_desa']); echo "</td>
        <td >"; echo getbidang($a_usulan['id_bidang']); echo "</td>
        <td >$a_usulan[detail_lokasi] </td>         
        <td>"; echo getdprd($a_usulan['id_dprd']); echo "</td>
        <td><input type='hidden' name='id' value='"; echo $p_id_kirim;  "'><br>
        <input type='hidden' name='id_usulan' value='"; echo $p_id_usulan = $a_usulan['id_usulan']; "'>
        <input type='hidden' name='admin' value='"; echo $p_id_admin = 'admin'; "'>
        <input type='hidden' name='tgl_verifikasi' value='"; echo $p_tgl_verifikasi = date('Y-m-d');"'>
        <input type='hidden' name='stat_verifikasi' value='"; echo $p_stat_verifikasi = 'Terverifikasi'; "'></td>";
        ...
        //   </td>";
        // }
         // echo "<td class='center'><a href='11_usulanterkirim.php'><button class='btn btn-theme' type='submit' name = 'kirim_daftar' value='KIRIM' >Kirim</button></a></td>";
        //echo "<td class='center'><a data-toggle='modal' href='11_usulanterkirim.php#myModal' class='fa fa-check red bigger-150'> </a></td>";
        echo "<td class='center'><a href='11_usulanterkirim.php?mod=kirim&id=$a_usulan[id_kirim]' class='fa fa-check red bigger-150'> </a> </td>";
        if ($mod == "kirim") {
        $q_kirim = mysql_query("INSERT INTO t_verifikasi VALUES ('$p_id_kirim', 
                                                         '$p_id_usulan', 
                                                         '$p_id_admin', 
                                                         '$p_tgl_verifikasi', 
                                                         '$p_stat_verifikasi')");
  ...
  if ($q_kirim) {
    ...
    window.location=('11_usulanterkirim.php')
     ...
     $q_update = mysql_query("UPDATE t_usulterkirim SET stat_verifikasi = 'Verifikasi' 
    ...
    window.location=('11_usulanterkirim.php')
        ...
        // if ($a_usulan['stat_verifikasi']=='Belum Terverifikasi') {
        // echo "<a href='#' onclick=\"buka('6.usulankegiatan.php?id_usulan=$a_usulan[0]')\">Map</a>";
        //   } else {
        //     echo " |
        //   <a href='11_usulanterkirim.php#myModal' class='fa fa-check red bigger-150'> </a>";
        //   }        
        // echo "</tr>";
      }
    }
    ...
    <?php
    if ($p_tombol == "KIRIM") {
      $q_kirim_usulan = "INSERT INTO t_verifikasi VALUES ('$p_id_verifikasi', 
                                                         '$p_id_usulan', 
                                                         '$p_id_admin', 
                                                         '$p_tgl_verifikasi', 
                                                         '$p_status')";
    ...
    if ($q_kirim ) {   
       ...
       window.location=("11_usulanterkirim.php")
        ...
        $q_update = mysql_query("UPDATE t_usulterkirim SET status = 'Terkirim' 
          WHERE id_kirim = '$p_id_kirim' ");
        $q_daftar = mysql_query($q_update); 
        echo $q_update;
       }   else {
      echo "Gagal";
    } 
  //  $q_stat = mysql_query("INSERT INTO t_verifikasi VALUES ('$p_id_verifikasi', 
  //                                                        '$p_id_usulan', 
  //                                                        '$p_id_admin', 
  //                                                        '$p_tgl_verifikasi'='date(Y-m-d)', 
  //                                                        '$p_status' = 'Terverifikasi'  )");
  ...
  //   window.location=('11_usulanterkirim.php')
  //    </script>";
  //    $q_update = mysql_query("UPDATE t_usulterkirim SET stat_verifikasi = 'Terverifikasi' where id_kirim = '$p_id_kirim' ");
  //       $q_daftar = mysql_query($q_update); 
  //       echo $q_update;
  // } else {
  //   echo "<script>alert('Data Gagal Diverifikasi'). </script>";
      ...
      <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
              ...
              <a href="11_usulanterkirim.php"><button data-dismiss="modal" class="btn btn-theme" type="submit" name = "mod" value="kirim" >Kirim</button></a>
            </div>
        </div>
      </div>
  </div>
