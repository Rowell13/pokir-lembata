...
if ($tb_act == "Edit") {
          $display = "style='display: none'";
          $q_edit_fraksi = mysql_query("UPDATE t_admverifikasi SET id_bidang  = '$p_id_bidang',
                                                          nip        = '$p_nip',
                                                          nama       = '$p_nama',
                                                          email      = '$p_nip',
                                                          hp         = '$p_hp',
                                                          n_user     = '$p_username'
                                                          WHERE id_admin = '$p_noid'");
          if ($q_edit_fraksi) {
      ...
      $display = "style='display: none'";
      $q_edit_user = mysql_query("UPDATE t_skpd SET p_user = '$p_password' WHERE id_admin = '$p_noid'");
...
<section id="container">
    ...
    <section class="wrapper"><h3><a href="14_profileverifikasi.php"><i class="fa fa-angle-right">&nbsp Profil </i></a> </h3>
      <div class="row mb">
        <div class="content-panel">
          ...
          <tbody>
          <?php 
              $q_user  = mysql_query("SELECT * FROM t_admverifikasi ORDER BY id_admin ASC") or die (mysql_error());
              $j_data   = mysql_num_rows($q_user);
              if ($j_data == 0) {
                echo "<tbody><tr><td text-align='center' colspan='6'>-- Tidak Ada Data --</td></tr></tbody>";
              } else {      
            ...
            <?php 
              if ($mode_form == "edit") {
                 $display = "";
                 $q_edit_user = mysql_query("SELECT * FROM t_admverifikasi WHERE id_admin = '$id_user'");
